<template>
  <div>
    <div v-if="$page.props.flash?.type == 'success' && show" class="flex items-center justify-between mb-8 max-w-3xl bg-green-500 rounded">
    ...
    <div v-if="($page.props.flash?.type == 'error' || Object.keys($page.props.errors).length > 0) && show" class="flex items-center justify-between mb-8 max-w-3xl bg-red-500 rounded">
      <div class="flex items-center">
        ...
        <div v-if="$page.props.flash?.message" class="py-4 text-white text-sm font-medium">{{ $page.props.flash?.message }}</div>
